...
dt$palette <- 'gray70'  # set default color 
# define color pallete: cut-off: p < 0.05, fold change: for transcriptome is 2 fold, while proteome is 1.2, logFC = 0.2630344
 for (i in 1:length(dt$palette)) {
  ...
  {dt$palette[i] <- 'red'  }
  # else if (dt$pro_padj[i] < 0.05 & dt$RNA_padj[i] < 0.05 & abs(as.numeric(dt$RNA_log2FC[i])) < 1 & abs(as.numeric(dt$pro_log2FC[i])) > 1) 
  # {dt$palette[i] <- 'blue'}
  # else 
  # {dt$palette[i] <- 'gray70' }
}
# plot pionts color by color
gray.points <- dt[which(dt$palette == 'gray70'),]
# blue.points <- dt[which(dt$palette == 'blue'),]
red.points <- dt[which(dt$palette == 'red'),]
pdf(file = paste(outpath,'quadrant.pdf', sep = ""),width = 10, height =8)
...
# points(blue.points$pro_log2FC~blue.points$RNA_log2FC,
#        col=blue.points$palette,
#        pch=16,
#        cex=1.5
# ) 
...
write.table(dt, file = paste(outpath, "quadrantData.txt", sep = ""))
...
## next step analysis: enrichment/pathway analysis, therapeutic targets & drugs 
